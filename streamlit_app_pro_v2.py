...
# Headless-safe Matplotlib config (font cache dir)
...
def analyze_drivers(values, features, composite, comps, weights, score, CONFIG):
    ...
    recs = []
    if score < 75:
        # target composition when aiming score 75 (not shown to user here)
        pass
    else:
...
# =========================
# Visuals (Plotly donuts without '%' anywhere)
# =========================
def donut_plotly(value_0_100: float, title: str):
    ...
    fig = go.Figure(data=[go.Pie(
        ...
        labels=["", ""],          # sem rótulos
        hole=0.7,
        textinfo="none",          # não mostrar porcentagem
        sort=False,
        direction="clockwise",
        showlegend=False
    )])
...
# App
# =========================
st.set_page_config(page_title="Temperatura da Pauta — Pro v2", page_icon="🔥", layout="wide")
st.title("🔥 Temperatura da Pauta — Pro v2")

# Sidebar (metas e pesos)
...
# ==== 4 donuts lado a lado (sempre 0-100; sem % em rótulo) ====
st.subheader("Pilares — Visão 0 a 100")
# Transformações para 0-100
men_0100 = pct_of_goal(menc_total, meta_menc)                   # progresso vs meta
eng_0100 = pct_of_goal(eng_total, meta_eng)                     # progresso vs meta
sent_0100 = float(np.clip(sent, 0, 100))                        # já é 0-100
bfit_0100 = float(np.clip(brand * 10.0, 0, 100))                # 0-10 -> 0-100
...
# Rodapé
st.markdown("---")
st.caption("Creative Data • Temperatura da Pauta — Pro v2 (donuts por variável)")
